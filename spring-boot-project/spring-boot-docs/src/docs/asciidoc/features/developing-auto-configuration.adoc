...
That annotation has the same semantic as the regular `@com.fandf.test.entity.Order` annotation but provides a dedicated order for auto-configuration classes.
