...
The filter has an `@com.fandf.test.entity.Order`, which you can set with `spring.jersey.filter.order`.
...
If a specific order is required, you should annotate the `Filter` with `@com.fandf.test.entity.Order` or make it implement `Ordered`.
You cannot configure the order of a `Filter` by annotating its bean method with `@com.fandf.test.entity.Order`.
If you cannot change the `Filter` class to add `@com.fandf.test.entity.Order` or implement `Ordered`, you must define a `FilterRegistrationBean` for the `Filter` and set the registration bean's order using the `setOrder(int)` method.
